...
public class CaixaImpl implements SistemaSupermercadoInterface {
    ...
    @Override
    public Collection<Produto> listarProdutos() throws EstoqueVazioException {
        if (estoque.isEmpty()) {
    ...
    public void finalizarCompra() throws EstoqueVazioException, PagamentoInvalidoException {
        if (carrinho == null || carrinho.isEmpty()) {
            throw new EstoqueVazioException("O carrinho está vazio. Adicione produtos antes de finalizar a compra.");
        }
        if (tipoPagamento == null) {
            throw new PagamentoInvalidoException("Tipo de pagamento não definido. Defina um tipo de pagamento antes de finalizar a compra.");
        }
        carrinho.clear();
    }
